        ...
        <div class="min-h-screen bg-slate-900">
            <div class="bg-slate-800 border-b border-slate-700 p-6">
                <div class="max-w-7xl mx-auto">
                    ...
                    <p class="text-slate-400">
                        Manage post-specific knowledge for AI replies.
                    </p>
                </div>
            </div>
                ...
                <div v-else class="space-y-4">
                    ...
                    >
                        <div class="flex justify-between gap-4 mb-4">
                            ...
                            <span
                                class="bg-slate-700 text-slate-200 text-center px-3 py-1 rounded-full text-xs h-fit"
                            >
                                {{
                                    post.knowledge_sources?.length || 0
                                }}
                                sources
                            </span>
                        </div>
                        ...
                        <div class="border-t border-slate-700 pt-4">
                            ...
                            <div
                                v-if="post.knowledge_sources?.length"
                                class="grid grid-cols-1 md:grid-cols-2 gap-2"
                            >
                                <div
                                    v-for="source in post.knowledge_sources"
                                    :key="source.id"
                                ...
                                >
                                    <div>
                                        ...
                                        <p
                                            class="text-slate-500 text-xs uppercase"
                                        >
                                            {{ source.type }} ·
                                            {{ source.scope || "global" }}
                                        </p>
                                    ...
                                    <LoadingButton
                                        ...
                                        @click="unlinkSource(post, source)"
                                    >
                                        Unlink
                                    </LoadingButton>
                                </div>
                            ...
                            <p v-else class="text-slate-500 text-sm">
                                No knowledge source linked.
                            </p>
                        </div>

                        <div class="flex flex-wrap gap-2 mt-5">
                            <LoadingButton
                                :loading="loading.attach[post.id]"
                                :loading-text="'Opening...'"
                                base-classes="bg-blue-600 hover:bg-blue-700 text-white text-sm rounded"
                                @click="openAttachModal(post)"
                            >
                                Attach Existing Source
                            </LoadingButton>

                            <LoadingButton
                                :loading="loading.upload[post.id]"
                                :loading-text="'Uploading...'"
                                base-classes="bg-green-700 hover:bg-green-800 text-white text-sm rounded"
                                @click="openUploadForPost(post)"
                            >
                                Upload Source For This Post
                            </LoadingButton>

                            <LoadingButton
                                base-classes="bg-slate-700 hover:bg-slate-600 text-white text-sm rounded"
            ...
            >
                ...
                >
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        ...
                        <LoadingButton
                            :loading="loading.attach[attachPost.id]"
            ...
            >
                ...
                >
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-xl font-bold text-white">
                            Upload Source For Post
                        </h2>
                        <button
                            @click="uploadPost = null"
                            class="text-slate-400 text-2xl"
                        >
                            ×
                        </button>
                    ...
                    <form
                        @submit.prevent="uploadSourceForPost"
                        class="space-y-4"
                    >
                        <input
                        ...
                        <div class="flex justify-end gap-2">
                            ...
                            <LoadingButton
                                :loading="loading.upload[uploadPost?.id]"
                                :loading-text="'Uploading...'"
                                base-classes="px-4 py-2 bg-green-700 hover:bg-green-800 text-white rounded"
                                type="submit"
                            >
                                Upload & Link
                            </LoadingButton>
            ...
            >
                ...
                >
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            ...
                            <p class="text-slate-400 text-sm">
                                ...
                                {{ detailPost.platform_post_id }}
                            </p>
                        </div>
                        <button
                            @click="detailPost = null"
                            class="text-slate-400 text-2xl"
                        >
                            ×
                        </button>
                    ...
                    <div class="space-y-2">
                        <div
                            v-for="source in detailPost.knowledge_sources || []"
                            :key="source.id"
...
const openAttachModal = (post) => {
    ...
    selectedSourceIds.value =
        post.knowledge_sources?.map((source) => source.id) || [];
    loading.attach[post.id] = true;
    setTimeout(() => {
        loading.attach[post.id] = false;
    }, 300);
};

const attachSources = () => {
    loading.attach[attachPost.value.id] = true;
    router.post(
        `/social-posts/${attachPost.value.id}/knowledge-sources`,
        {
        ...
        },
        {
            ...
            onSuccess: () => {
                ...
                toast.success("Sources attached successfully");
            },
            onFinish: () => {
                loading.attach[attachPost.value.id] = false;
            },
        },
    );
};
...
const uploadSourceForPost = () => {
    loading.upload[uploadPost.value.id] = true;
    router.post(
        `/social-posts/${uploadPost.value.id}/knowledge-sources/upload`,
        {
            ...uploadForm.value,
            scope: "post_specific",
        },
        {
            ...
            onSuccess: () => {
                ...
                toast.success("Source uploaded & linked");
                uploadForm.value = {
                    name: "",
                    description: "",
                    type: "",
            ...
            onFinish: () => {
                loading.upload[uploadPost.value.id] = false;
            },
        },
    );
};
