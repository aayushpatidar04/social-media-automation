      ...
      <div class="max-w-7xl mx-auto p-6">
        <div class="bg-slate-800 border border-slate-700 rounded-lg p-4 mb-6">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <select v-model="filters.type" @change="applyFilters" class="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white">
            ...
            <select v-model="filters.scope" @change="applyFilters" class="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white">
              <option value="">All Scope</option>
              <option value="global">Global</option>
              <option value="post_specific">Post Specific</option>
            </select>

            <select v-model="filters.indexed" @change="applyFilters" class="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white">
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          >
            <div class="flex justify-between gap-4">
              ...
              <span
                :class="source.is_indexed ? 'bg-green-900 text-green-200' : 'bg-yellow-900 text-yellow-200'"
            ...
            <div class="flex flex-wrap gap-2 mt-4">
              <button @click="openLinkModal(source)" class="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded">
                Link Posts
              </button>

              <button @click="reindexSource(source)" class="px-3 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm rounded">
                Re-index
              </button>

              <button @click="deleteSource(source)" class="px-3 py-2 bg-red-700 hover:bg-red-800 text-white text-sm rounded">
                Delete
              </button>
            </div>
          </div>
      ...
      <div v-if="showUploadModal" class="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
        <div class="bg-slate-800 border border-slate-700 rounded-lg max-w-2xl w-full p-6">
          ...
          <form @submit.prevent="uploadSource" class="space-y-4">
            <input v-model="form.name" required placeholder="Source name" class="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded text-white" />

            <textarea v-model="form.description" placeholder="Description" class="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded text-white"></textarea>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select v-model="form.type" required class="px-4 py-2 bg-slate-700 border border-slate-600 rounded text-white">
              ...
              <select v-model="form.scope" required class="px-4 py-2 bg-slate-700 border border-slate-600 rounded text-white">
                <option value="global">Global</option>
                <option value="post_specific">Post Specific</option>
              </select>
            </div>
            ...
            <div class="flex justify-end gap-2">
              ...
              <button type="submit" :disabled="uploading" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded">
